...
/**
 * Test implementation of BatchConsumer using vitest mocks.
 *
 ...
 * TestChannel respectively. `close` method is a vitest mock returning void.
 *
...
export class TestBatchConsumer<T> extends EventEmitter implements BatchConsumer<T> {
    ...
    async deliverMessages(
        ...
        const callback = lastCall !== undefined ? lastCall[0] as BatchConsumerCallbackFn<T> : undefined;
        if (!callback)
